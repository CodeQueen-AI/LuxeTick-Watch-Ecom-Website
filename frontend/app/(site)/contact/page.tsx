...
export default function ContactPage() {
  ...
  return (
    <section className="w-full min-h-screen flex flex-col md:flex-row poppins">
            ...
            <div className="flex justify-center mt-4">
              <button
                ...
                className="w-full sm:w-48 bg-black text-white border border-white py-3
                  hover:bg-white hover:text-black hover:border-black transition cursor-pointer disabled:opacity-50 rounded"
              >
                {loading ? "Sending..." : "Send Message"}
              </button>
            </div>
      ...
      {/* TOAST NOTIFICATION */}
      {toast && (
        <div className="fixed bottom-6 right-6 flex items-center gap-3 bg-white shadow-xl px-5 py-3 z-50 rounded-md">
          {type === "success" ? (
            <AiOutlineCheckCircle className="text-green-500 text-2xl" />
          ) : (
            <AiOutlineCloseCircle className="text-red-500 text-2xl" />
