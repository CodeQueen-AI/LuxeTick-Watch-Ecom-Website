...
export default function WishlistPage() {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-10 py-10 poppins">
      {/* Toast */}
      {toast && (
      ...
      {wishlistItems.length === 0 ? (
        ...
        <>
          {/* Heading */}
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extralight allura text-center mb-10 flex flex-col sm:flex-row items-center justify-center gap-3">
            <AiFillHeart className="text-pink-500 text-4xl sm:text-4xl" />
            My Wishlist
          </h1>

          {/* Table */}
          <div className="overflow-x-auto">
            <table className="w-full border-collapse min-w-[400px] md:min-w-full">
              <thead className="border-b border-gray-300 text-left text-sm uppercase">
                <tr>
                  <th className="pb-4">Image</th>
                  <th className="pb-4">Name</th>
              ...
              <tbody>
                {wishlistItems.map((item) => (
                  <tr key={item.id} className="border-b border-gray-300 hover:shadow-sm transition">
                    ...
                    <td className="py-4 font-medium">{item.name}</td>
                    <td className="py-4">${item.price}</td>
                    <td className="py-4 pl-0 sm:pl-4">
                      <button
                        onClick={() => handleAddCart(item)}
                        className="px-3 py-1 bg-black text-white text-sm hover:bg-white hover:text-black border border-black transition cursor-pointer w-full sm:w-auto text-center">
                        Add to Cart
                      </button>
                    </td>
                    <td className="py-4">
                      <button
                        onClick={() => handleRemove(item)}
                        className="text-red-500 hover:text-red-600 transition cursor-pointer text-xl sm:text-base">
                        <FaTrash />
                      </button>
                    </td>
                  </tr>
