...
//     if (tagFilters.newArrival) filtered = filtered.filter(p => p.newArrival);
//     if (tagFilters.outOfStock) filtered = filtered.filter(p => p.stock === 0);
...
//               <div key={product.id} className={`border p-5 hover:shadow-lg transition relative flex flex-col justify-between ${product.stock === 0 ? "opacity-50 pointer-events-none" : ""}`}>
...
//                     {product.stock === 0 && <span className="text-[10px] font-medium px-2 py-[2px] rounded-full bg-gray-800 text-white shadow-sm uppercase tracking-wide">Out Of Stock</span>}
...
//                     {product.newArrival && <span className="text-[10px] font-medium px-2 py-[2px] rounded-full bg-green-500 text-white shadow-sm uppercase tracking-wide">New</span>}
//                   </div>

//                   <Link href={`/products/${product.id}`} className={product.stock === 0 ? "pointer-events-none" : ""}>
//                     <Image src={product.image} alt={product.name} fill className="object-contain" />
//                   </Link>

//                   {/* Wishlist & Cart */}
//                   <div className={`absolute top-2 right-2 flex flex-col gap-2 transition ${product.stock === 0 ? "opacity-0" : "opacity-0 group-hover:opacity-100"}`}>
...
//   const toggleTagFilter = (tag: string) => setTagFilters(prev => ({ ...prev, [tag]: !prev[tag] }));
...
//     setSelectedFilters(updatedFilters);
//   };
//   const removeFilter = (filter: any) => setSelectedFilters(selectedFilters.filter(f => f !== filter));
//   const clearAllFilters = () => {
//     setSelectedFilters([]);
//     setTagFilters({ discount: false, newArrival: false, outOfStock: false });
//     setFilteredProducts(products);
...
//           <div className={`grid gap-8 flex-1 transition-all duration-500 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3`}>
//             {currentProducts.map((product) => (
//               <div key={product._id} className={`border p-5 hover:shadow-lg transition relative flex flex-col justify-between ${product.stock === 0 ? "opacity-50 pointer-events-none" : ""}`}>
//                 <div className="relative h-64 w-full bg-gray-100 group cursor-pointer overflow-hidden">
//                   {/* Tags */}
//                   <div className="absolute top-2 left-2 flex flex-col gap-1 z-50">
//                     {product.stock === 0 && <span className="text-[10px] font-medium px-2 py-[2px] rounded-full bg-gray-800 text-white shadow-sm uppercase tracking-wide">Out Of Stock</span>}
...
//                   <Link href={`/products/${product._id}`} className={product.stock === 0 ? "pointer-events-none" : ""}>
//                     <Image src={product.image} alt={product.name} fill className="object-contain" />
//                   </Link>
...
//                   <div className={`absolute top-2 right-2 flex flex-col gap-2 transition ${product.stock === 0 ? "opacity-0" : "opacity-0 group-hover:opacity-100"}`}>
//                     <button onClick={() => handleWishlist(product)} className="bg-white p-3 rounded-full shadow hover:bg-gray-200 transition cursor-pointer">
//                       <FiHeart className={`text-xl ${wishlistItems.find(item => item._id === product._id) ? "text-red-500" : "text-gray-500"}`} />
//                     </button>
//                     <button onClick={() => handleAddCart(product)} className="bg-white p-3 rounded-full shadow hover:bg-gray-200 transition cursor-pointer">
//                       <BsCart3 className="text-xl text-gray-700" />
//                     </button>
...
//   const [expanded, setExpanded] = useState<any>({});
//   const [toast, setToast] = useState("");
//   const [openFilter, setOpenFilter] = useState(true); // Always open for large screens
...
//   return (
//     <section className="w-full min-h-screen px-6 md:px-12 py-12 poppins relative">
//       {/* Toast */}
//       {toast && (
//         <div className="fixed bottom-6 right-6 flex items-center gap-3 bg-white border border-gray-200 shadow-xl px-5 py-3 animate-slideIn z-50">
//           <AiOutlineCheckCircle className="text-green-500 text-2xl" />
...
//       <h1 className="text-5xl md:text-7xl font-extralight text-center mb-12 allura">Our Products</h1>

//       <div className="flex gap-10">
//         {/* Filter Sidebar (only large screens) */}
//         <div className="hidden lg:block w-80">
//           <div className="border p-6 bg-white max-h-[1200px]">
//             <div className="flex justify-between items-center mb-4">
//               <p className="text-sm text-gray-400 cursor-pointer hover:underline" onClick={clearAllFilters}>Clear All Filters</p>
//             </div>
...
//   {/* Discount Tag - only if discount > 0 */}
//   {product.discount > 0 && (
...
//   {/* New Arrival Tag */}
//   {product.newArrival && (
//     <span className="text-[10px] font-medium px-2 py-[2px] rounded-full bg-green-500 text-white shadow-sm uppercase tracking-wide">
//       New
//     </span>
//   )}
// </div>
...
//         {/* Filter Sidebar */}
//         <div className="hidden lg:block w-80">
//           {/* ... Filter sidebar same rahega ... */}
//           <div className="border p-6 bg-white max-h-[1200px]">
//             {/* (Filter code yahan same hai - maine space save karne ke liye chhoda hai) */}
//             {/* Agar pura filter code chahiye to batao */}
//           </div>
//         </div>
...
//                 <div className="relative h-64 w-full bg-gray-100 group cursor-pointer overflow-hidden">
//                   <div className="absolute top-2 left-2 flex flex-col gap-1 z-50">
//                     {product.stock === 0 && <span className="text-[10px] font-medium px-2 py-[2px] rounded-full bg-gray-800 text-white">Out Of Stock</span>}
//                     {product.discount > 0 && <span className="text-[10px] font-medium px-2 py-[2px] rounded-full bg-red-500 text-white">{product.discount}% OFF</span>}
//                     {product.newArrival && <span className="text-[10px] font-medium px-2 py-[2px] rounded-full bg-green-500 text-white">New</span>}
//                   </div>
...
//                   <div className={`absolute top-2 right-2 flex flex-col gap-2 transition ${product.stock === 0 ? "opacity-0" : "opacity-0 group-hover:opacity-100"}`}>
//                     <button onClick={() => handleWishlist(product)} className="bg-white p-3 rounded-full shadow hover:bg-gray-200">
//                       <FiHeart className={`text-xl ${wishlistItems.find(item => item._id === product._id) ? "text-red-500" : "text-gray-500"}`} />
//                     </button>
//                     <button onClick={() => handleAddCart(product)} className="bg-white p-3 rounded-full shadow hover:bg-gray-200">
//                       <BsCart3 className="text-xl text-gray-700" />
//                     </button>
//                   </div>
//                 </div>
...
//                   <div className="flex justify-between items-center">
//                     <Link href={`/products/${product._id}`} className="bg-black text-white px-3 py-1 text-sm hover:bg-gray-800">
//                       View 
//                     </Link>
//                     <p className="font-serif text-xl">${product.price}</p>
//                   </div>
...
//                     Stock: <span className="font-medium text-black">{product.stock}</span>
//                   </div>
//                 </div>
//               </div>
//             ))}
...
//               className={`px-3 py-1 border ${currentPage === i + 1 ? "bg-black text-white border-black" : "bg-white text-gray-700 border-gray-300"}`}
//             >
//               {i + 1}
//             </button>
//           ))}
...
export default function ProductsPage() {
  ...
  const applyFilters = () => {
    ...
    if (tagFilters.discount) filtered = filtered.filter(p => p.discount > 0);
    if (tagFilters.newArrival) filtered = filtered.filter(p => p.newArrival);
    if (tagFilters.outOfStock) filtered = filtered.filter(p => p.stock === 0);
    ...
    selectedFilters.forEach(f => {
      switch (f.category) {
        case "Brands":
          ...
          break;
        case "Colors":
          filtered = filtered.filter(p => p.color === f.value);
          break;
        case "Strap Type":
          filtered = filtered.filter(p => p.strap === f.value);
          break;
        case "Category / Style":
          filtered = filtered.filter(p => p.category === f.value);
          break;
        case "Price":
          switch (f.value) {
            case "$100–$200":
              ...
              break;
            case "$201–$350":
              filtered = filtered.filter(p => p.price >= 201 && p.price <= 350);
              break;
            case "$351–$500":
              filtered = filtered.filter(p => p.price >= 351 && p.price <= 500);
              break;
            case "$501+":
              filtered = filtered.filter(p => p.price >= 501);
              break;
          }
          break;
      }
  ...
  return (
    <section className="w-full min-h-screen px-6 md:px-12 py-12 poppins relative">
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 flex items-center gap-3 bg-white border border-gray-200 shadow-xl px-5 py-3 z-50 rounded-lg">
          <AiOutlineCheckCircle className="text-green-500 text-2xl" />
          <p className="text-sm">{toast}</p>
        </div>
      )}
      ...
      <div className="flex gap-10">
        {/* Filter Sidebar */}
        <div className="hidden lg:block w-80">
          <div className="border p-6 bg-white sticky top-6">
            <div className="flex justify-between items-center mb-4">
              <p className="text-sm text-gray-400 cursor-pointer hover:underline" onClick={clearAllFilters}>
                Clear All Filters
              </p>
            </div>
            ...
            <div className="flex gap-2 mb-6 flex-wrap">
              <span onClick={() => toggleTagFilter("discount")} className={`cursor-pointer px-3 py-1 text-xs rounded-full font-medium transition ${tagFilters.discount ? "bg-red-700 text-white" : "bg-gray-100 text-gray-700 hover:bg-red-200"}`}>Discount</span>
              <span onClick={() => toggleTagFilter("newArrival")} className={`cursor-pointer px-3 py-1 text-xs rounded-full font-medium transition ${tagFilters.newArrival ? "bg-green-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-green-200"}`}>New Arrival</span>
              <span onClick={() => toggleTagFilter("outOfStock")} className={`cursor-pointer px-3 py-1 text-xs rounded-full font-medium transition ${tagFilters.outOfStock ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-700 hover:text-white"}`}>Out of Stock</span>
            </div>
            ...
            {selectedFilters.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {selectedFilters.map((f, idx) => (
                  <div key={idx} className="flex items-center bg-gray-200 text-gray-700 text-xs px-3 py-1 rounded-full">
                    {f.value}
                    <FiX className="ml-2 cursor-pointer" onClick={() => removeFilter(f)} />
                  </div>
                ))}
              </div>
            )}
            ...
            {Object.keys(filterOptions).map((category) => (
              <div key={category} className="mb-6">
                <div className="flex justify-between items-center cursor-pointer py-2 border-b" onClick={() => toggleExpand(category)}>
                  <span className="font-medium">{category}</span>
                  {expanded[category] ? <FiMinus /> : <FiPlus />}
                </div>
                <div className={`transition-all duration-300 overflow-hidden ${expanded[category] ? "max-h-96 mt-3" : "max-h-0"}`}>
                  {category === "Price" ? (
                    <div className="flex flex-wrap gap-2">
                      {filterOptions.Price.map((range) => {
                        const selected = selectedFilters.some(f => f.category === "Price" && f.value === range);
                        return (
                          <div key={range} onClick={() => toggleFilter("Price", range)} className={`cursor-pointer px-4 py-2 rounded-full border text-sm ${selected ? "bg-black text-white border-black" : "border-gray-300 hover:bg-gray-100"}`}>
                            {range}
                          </div>
                        );
                      })}
                    ...
                    filterOptions[category as keyof typeof filterOptions].map((option) => {
                      const selected = selectedFilters.some(f => f.category === category && f.value === option);
                      return (
                        <div key={option} className={`flex items-center gap-2 cursor-pointer py-2 text-sm ${selected ? "text-black font-medium" : "text-gray-600"}`} onClick={() => toggleFilter(category, option)}>
                          <div className={`w-4 h-4 border rounded-full ${selected ? "bg-black" : "bg-white"}`} />
                          {option}
                        </div>
                      );
                    })
            ...
            <button className="w-full bg-black text-white py-3 mt-4 cursor-pointer hover:bg-gray-900 transition" onClick={applyFilters}>
              APPLY FILTERS
            </button>
          </div>
        </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="flex flex-col justify-center items-center flex-1 py-40 gap-4">
            <AiOutlineProduct size={60} className="text-gray-400" />
            <p className="text-xl font-medium">No Product Found</p>
          </div>
        ) : (
          <div className="flex-1">
            <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {currentProducts.map((product) => (
                <div key={product._id} className={`border p-5 hover:shadow-lg transition-all relative flex flex-col justify-between ${product.stock === 0 ? "opacity-60 pointer-events-none" : ""}`}>
                  
                  <div className="relative h-64 w-full bg-gray-100 group cursor-pointer overflow-hidden rounded-lg">
                    <div className="absolute top-3 left-3 flex flex-col gap-1 z-50">
                      {product.stock === 0 && <span className="text-[10px] px-2 py-1 bg-gray-800 text-white rounded-full">Out Of Stock</span>}
                      {product.discount > 0 && <span className="text-[10px] px-2 py-1 bg-red-500 text-white rounded-full">{product.discount}% OFF</span>}
                      {product.newArrival && <span className="text-[10px] px-2 py-1 bg-green-500 text-white rounded-full">NEW</span>}
                    </div>
                    ...
                    <div className={`absolute top-3 right-3 flex flex-col gap-2 transition ${product.stock === 0 ? "opacity-0" : "opacity-0 group-hover:opacity-100"}`}>
                      <button onClick={() => handleWishlist(product)} className="bg-white p-3 rounded-full shadow hover:bg-gray-100">
                        <FiHeart className={`text-xl ${wishlistItems.find(item => item._id === product._id) ? "text-red-500" : "text-gray-500"}`} />
                      </button>
                      <button onClick={() => handleAddCart(product)} className="bg-white p-3 rounded-full shadow hover:bg-gray-100">
                        <BsCart3 className="text-xl text-gray-700" />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="mt-5">
                    ...
                    <div className="flex justify-between items-center mt-3">
                      <Link href={`/products/${product._id}`} className="bg-black text-white px-4 py-2 text-sm hover:bg-gray-800 transition">
                        View 
                      </Link>
                      <p className="font-serif text-2xl font-light">${product.price}</p>
                    </div>
              ...
              <div className="flex justify-center gap-2 mt-12">
                {Array.from({ length: totalPages }, (_, i) => (
                  <button
                    key={i}
                    onClick={() => setCurrentPage(i + 1)}
                    className={`px-4 py-2 border transition ${currentPage === i + 1 ? "bg-black text-white border-black" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"}`}
                  >
                    {i + 1}
                  </button>
                ))}
