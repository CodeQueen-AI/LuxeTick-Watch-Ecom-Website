...
//     const exists = updatedFilters.find(
...
//     } else {
//       updatedFilters = [
//         ...updatedFilters.filter((f) => f.category !== category),
//         { category, value },
//       ];
//     }
...
//             case "$201–$350":
//               filtered = filtered.filter(
//                 (p) => p.price >= 201 && p.price <= 350
//               );
//               break;

//             case "$351–$500":
//               filtered = filtered.filter(
//                 (p) => p.price >= 351 && p.price <= 500
//               );
//               break;
...
//         case "Gender":
//           filtered = filtered.filter((p) => p.gender === f.value);
//           break;
//       }
...
//       <h1 className="text-7xl font-extralight text-center mb-12 allura">
//         Our Products
//       </h1>
...
//         >
//           <div
//             className={`border p-6 bg-white relative transition-all duration-500 ${
//               openFilter ? "max-h-[1200px]" : "max-h-0"
//             }`}
//           >
...
//                 <div
//                   key={idx}
//                   className="flex items-center bg-gray-200 text-gray-700 text-xs px-2 py-1 rounded-full"
//                 >
//                   {f.value}
//                   <FiX
...
//                     expanded === category ? "max-h-64 mt-2" : "max-h-0"
//                   }`}
//                 >
...
//                           (f) => f.category === "Price" && f.value === range
//                         );
...
//             <button
//               className="w-full bg-black text-white py-3 mt-4 cursor-pointer"
...
//         <div className="grid gap-8 flex-1 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
...
//                 <Link href={`/products/${product.id}`}>
//                   <button className="bg-black text-white px-6 py-2 border border-black hover:bg-white hover:text-black transition cursor-pointer">
//                     View
//                   </button>
//                 </Link>
...
const productsData = [
  { id: 9, name: "Black Leather", brand: "Rolex", color: "Black", price: 250, strap: "Leather", category: "Luxury", gender: "Men", img: "/products/p1.jpg", inStock: true },
  { id: 10, name: "Rose Gold", brand: "Casio", color: "Rose Gold", price: 350, strap: "Metal", category: "Classic", gender: "Women", img: "/products/p2.jpg", newArrival: true },
  { id: 11, name: "White Dial", brand: "Fossil", color: "White", price: 200, strap: "Silicone", category: "Sport", gender: "Men", img: "/products/p3.jpg", discount: 20 },
  { id: 12, name: "Silver Chrono", brand: "Seiko", color: "Silver", price: 450, strap: "Metal", category: "Smart Watch", gender: "Women", img: "/products/p4.jpg" },
...
export default function ProductsPage() {
  ...
  const removeFilter = (filter) => setSelectedFilters(selectedFilters.filter(f => f !== filter));
  const clearAllFilters = () => { setSelectedFilters([]); setFilteredProducts(productsData); };
  ...
  return (
    <section className="w-full min-h-screen px-12 py-12 poppins relative">
      {toast && (
        <div className="fixed bottom-6 right-6 flex items-center gap-3 bg-white border border-gray-200 shadow-xl px-5 py-3 z-50">
          <AiOutlineCheckCircle className="text-green-500 text-2xl" />
          <p className="text-sm"><span className="font-semibold">{toast}</span> Added To Cart!</p>
        </div>
      )}
      ...
      <div className="flex gap-10 items-start">
        ...
        {/* Sidebar */}
        <div className={`transition-all duration-500 overflow-hidden ${openFilter ? "w-80" : "w-0"}`}>
          <div className={`border p-6 bg-white relative transition-all duration-500 ${openFilter ? "max-h-[1200px]" : "max-h-0"}`}>
            <div className="flex justify-between items-center mb-4">
              <p className="text-sm text-gray-400 cursor-pointer hover:underline" onClick={clearAllFilters}>Clear All Filters</p>
              <button onClick={() => setOpenFilter(false)} className="cursor-pointer"><FiX size={20} /></button>
            </div>
            ...
            {Object.keys(filterOptions).map((category, idx) => (
              <div key={idx} className="mb-4">
                ...
                <div className={`transition-all duration-500 ease-in-out overflow-hidden ${expanded === category ? "max-h-64 mt-2" : "max-h-0"}`}>
                  {category === "Price" ? (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {filterOptions.Price.map((range, idx) => {
                        const selected = selectedFilters.some(f => f.category === "Price" && f.value === range);
                        return (
                          <div key={idx} onClick={() => toggleFilter("Price", range)} className={`cursor-pointer px-3 py-1 rounded-full border ${selected ? "bg-black text-white border-black" : "bg-white text-gray-700 border-gray-300"}`}>
                            {range}
                          </div>
                        );
                      })}
                    ...
                    filterOptions[category].map((option, idx2) => {
                      const selected = selectedFilters.some(f => f.category === category && f.value === option);
                      return (
                        <div key={idx2} className="flex items-center gap-2 py-1 cursor-pointer" onClick={() => toggleFilter(category, option)}>
                          <div className={`w-4 h-4 border rounded-full flex items-center justify-center ${selected ? "bg-black" : "bg-white"}`}></div>
                          <span>{option}</span>
                        </div>
                      );
                    })
    ...
    <span className="bg-red-100 text-red-700 text-[10px] font-semibold px-3 py-1 rounded-full shadow-md transform transition-all duration-300 hover:scale-105">
      -{product.discount}%
                ...
                <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition">
                  ...
                  <button onClick={() => handleAddCart(product)} className="bg-white p-3 rounded-full shadow hover:bg-gray-200 transition"><BsCart3 className="text-gray-800 text-xl" /></button>
                </div>
              ...
              <div className="mt-2 flex justify-between items-center">
                <Link href={`/products/${product.id}`}>
                  <button className="bg-black text-white px-6 py-2 border border-black hover:bg-white hover:text-black transition cursor-pointer">View</button>
                </Link>
                <p className="font-serif text-2xl">${product.price}</p>
              </div>
            </div>
