...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // ⚡ BULLETPROOF FIX: handle any data shape
        setOrders(
          Array.isArray(data)
            ? data
  ...
  if (loading) return <div className="bg-white rounded-3xl p-12 text-center">
    <p className="text-xl text-zinc-600">Loading orders...</p>
  </div>;

  if (error) return <div className="bg-white rounded-3xl p-12 text-center">
    <p className="text-red-500 text-lg">{error}</p>
  </div>;
          ...
          <tbody>
            {Array.isArray(orders) && orders.length > 0 ? (
              orders.map((order) => {
                ...
                return (
                  <tr key={order._id} className="border-b border-zinc-100 hover:bg-zinc-50 transition-colors">
                    <td className="py-5 px-6 font-mono text-zinc-700">
                      {order.orderId || `#${order._id.slice(-8).toUpperCase()}`}
                    </td>
                    ...
                    <td className="py-5 px-6 text-zinc-500">
                      {new Date(order.createdAt || order.date || Date.now()).toLocaleDateString('en-PK', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric',
                        hour: '2-digit',
