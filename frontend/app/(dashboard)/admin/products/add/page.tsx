...
//     if (stockValue < 0) {
//       showToast("Stock cannot be less than 0", "error");
//       setLoading(false);
//       return;
//     }
...
//               className="w-full border border-gray-300 px-4 pt-3 pb-2 focus:outline-none focus:ring-2 focus:ring-[#09162c] shadow-sm"
//             />
//           </div>
//         </div>
...
export default function AddProductPage() {
  ...
  // Handle Stock Input - Prevent Negative Values
  const handleStockChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    let value = e.target.value;
    
    // Agar negative value daale to 0 kar do
    if (value.startsWith('-')) {
      e.target.value = '0';
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto mt-1 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              type="number"
              required
              min="0"           // HTML level protection
              onChange={handleStockChange}   // ← Real-time negative block
              className="w-full border border-gray-300 px-4 pt-3 pb-2 focus:outline-none focus:ring-2 focus:ring-[#09162c] shadow-sm"
            />
          </div>
        </div>
        ...
        <div>
          <label className="block text-gray-700 text-sm font-medium mb-1">Discount %</label>
          <input name="discount" type="number" min="0" className="w-full border border-gray-300 px-4 pt-3 pb-2 focus:outline-none focus:ring-2 focus:ring-[#09162c] shadow-sm"/>
        </div>
      ...
      {/* Toast Notification */}
      {toast && (
        <div className="fixed bottom-6 right-6 flex items-center gap-3 bg-white px-5 py-3 z-50 shadow-lg rounded-lg border">
          {type === "success" ? (
            <AiOutlineCheckCircle className="text-green-500 text-2xl" />
          ) : (
            <AiOutlineCloseCircle className="text-red-500 text-2xl" />
