...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const salesData = months.map(() => Math.floor(Math.random() * 300));
    const revenueData = months.map(() => Math.floor(Math.random() * 200));
    setLineSeries([
  ...
  const lineOptions = {
    ...
    colors: ["#4f46e5", "#10b981"],
    tooltip: { theme: "dark", shared: true },
    fill: { type: "gradient", gradient: { shade: "light", opacityFrom: 0.6, opacityTo: 0.1, stops: [0,100] } }
  };
