...
export const createOrder = async (req, res) => {
  try {
    ...
    await Notification.create({
      userId: req.user?.id,
      type: "order",
      message: `Your order for "${productName}" has been placed and is pending.`,
    });
...
export const createOrdersFromCart = async (req, res) => {
  try {
    ...
    const orders = await Promise.all(
      cartItems.map(async (item) => {
        ...
        await Notification.create({
          userId: req.user?.id,
          type: "order",
          message: `Your order for "${item.name}" has been placed and is pending.`,
        });
    ...
    res.status(201).json({
      success: true,
      message: "Orders created successfully and notifications sent",
      count: orders.length,
      orders,
    });
