...
import cloudinary from "../config/cloudinary.js";
import multer from "multer";
import Notification from "../../notifications/Models/notification.js"; // Notification model
import User from "../../users/Models/user.js"; // User model
...
export const createProduct = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Product added successfully and notifications sent", product });
  } catch (err) {
    console.error("Error in createProduct:", err);
    res.status(500).json({ message: err.message || "Server error" });
  }
...
// Toggle New Arrival (ignore notifications, only status change)
export const toggleNewArrival = async (req, res) => {
  try {
    const product = await Product.findById(req.params.id);
    if (!product) return res.status(404).json({ message: "Product not found" });
